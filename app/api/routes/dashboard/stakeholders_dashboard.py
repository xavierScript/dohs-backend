...
dashboard_router = APIRouter(
    prefix="/stakeholders/dashboard",
    tags=["Stakeholders Dashboard Analytics"]
)
...
def get_case_count_by_period(session: Session, model, start_date: datetime, end_date: datetime) -> int:
...
def get_recent_activities(session: Session,  limit: int = 3) -> List[Dict[str, Any]]:
    ...
    health_cases = session.exec(
        ...
        ).order_by(HealthCase.created_at.desc()).limit(limit)
    ).all()
    ...
    animal_cases = session.exec(
        ...
        ).order_by(AnimalCase.created_at.desc()).limit(limit)
    ).all()
    ...
    env_cases = session.exec(
        ...
        ).order_by(EnvironmentalCase.created_at.desc()).limit(limit)
    ).all()
...
async def get_dashboard(
    ...
    try:
        ...
        # Recent Activities
        recent_activities = get_recent_activities(session, 3)
